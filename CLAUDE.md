...
- **Sports and capacity**: `internal/sport` defines supported sports, default/max players per unit, unit kind, and emoji. Venues own `venue_sports`; games snapshot `sport` and resolved `players_per_court`, and capacity is `courts_count × players_per_court`. Eversports auto-booking is squash-only; 1v1 results require two players per unit.
- **Message formatting**: `internal/gameformat` produces sport-aware game announcement text and is shared by `management` and `telegram`.
