...
- `auto_booking_courts`: ordered comma-separated court IDs (subset of `courts`) that `RunAutoBooking` will attempt to book, in declared priority order. Empty means any available court is eligible (API response order). Validated: each ID must be present in `courts`. Only editable (via `venue_edit_auto_booking_courts` button) when `auto_booking_enabled = true`. Set in venue creation wizard only if auto-booking was enabled at the enable/disable step. **Note:** the `courts` field stores user-defined labels (e.g. `1,2,3`) while Eversports uses facility-specific numeric IDs (e.g. `77385,77386`). When neither `courts` nor `auto_booking_courts` match Eversports IDs, `filterAvailableCourts` falls back to booking all available courts returned by the API, ignoring priority ordering. The booking still succeeds; priority selection only works if the stored IDs match Eversports court IDs.
...
- **`AutoBookingJob`** (`auto_booking.go`): Iterates all groups. For each group, checks if local time is in the `[00:00, 00:05)` window. For each venue with `auto_booking_enabled = true` and `game_days` and `preferred_game_time` configured, checks if today's weekday is in `game_days`. Deduplicates via `venues.last_auto_booking_at` (date-scoped). If conditions met and `bookingClient` is set:
  ...
  3. Selects free courts via `filterFreeCourts`: prefers courts in `courts` (venue-scoped filter); if none of the court IDs from `ListCourts` match `courts` (e.g. venue stores sequential labels while Eversports uses facility-specific IDs), falls back to all free courts. Within the eligible set, if `auto_booking_courts` is set and any preferred court is free, returns those in declared priority order; otherwise returns all eligible free courts in API response order.
