...
- **Message formatting**: `internal/gameformat` produces game announcement text (emoji header, player list, capacity line `courts_count × 2`, "Last updated" footer, "Game completed ✓" marker). Used by `management` (GameNotifier) and `telegram` (formatter.go). `GameNotifier` serializes the full fetch-render-send sequence per game so an older in-flight edit cannot overwrite newer court state. Standard join/skip/guest mutations rely on the management notifier only; the telegram service must not issue a duplicate edit for those callbacks.
...
- **Preventive cancellation**: `venues.preventive_cancellation_fraction` is `1/3`, `1/2` (default), or `2/3`; the Web-only control schedules the check from group-local booking-opening midnight to the grace-period deadline. Telegram venue updates omit and preserve it.
