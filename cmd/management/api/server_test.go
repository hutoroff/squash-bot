...
func TestLimitRequestBody_AllowsBodyUnderLimit(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/games", bytes.NewReader([]byte(`{"hello":"world"}`)))
	w := httptest.NewRecorder()
	h.ServeHTTP(w, req)
	if w.Code != http.StatusOK {
...
func TestLimitRequestBody_RejectsBodyOverLimit(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/games", bytes.NewReader(oversized))
	w := httptest.NewRecorder()
	h.ServeHTTP(w, req)
