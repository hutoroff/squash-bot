...
func TestListAuditEvents_ZeroCallerHeader(t *testing.T) {
	h, _ := newAuditHandler()
	req := httptest.NewRequest(http.MethodGet, "/api/v1/audit", nil)
	req.Header.Set("X-Caller-User-Id", "0")
	w := httptest.NewRecorder()
	h.listAuditEvents(w, req)
...
func TestListAuditEvents_InvalidCallerHeader(t *testing.T) {
	h, _ := newAuditHandler()
	req := httptest.NewRequest(http.MethodGet, "/api/v1/audit", nil)
	req.Header.Set("X-Caller-User-Id", "notanumber")
	w := httptest.NewRecorder()
	h.listAuditEvents(w, req)
...
func TestListAuditEvents_NonOwner_RestrictsToOwnEvents(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/audit", nil)
	req.Header.Set("X-Caller-User-Id", "123")
	w := httptest.NewRecorder()
	h.listAuditEvents(w, req)
	...
	f := repo.captured[0]
	if f.OwnUserID == nil || *f.OwnUserID != 123 {
		t.Errorf("OwnUserID: want 123, got %v", f.OwnUserID)
	}
	...
	if f.ActorUserID != nil {
		t.Errorf("ActorUserID: should be nil in non-owner path, got %v", f.ActorUserID)
	}
...
func TestListAuditEvents_NonOwner_AdminResolverError_GracefulDegradation(t *testing.T) {
	// Resolver fails — caller should still see own events via OwnUserID
...
func TestListAuditEvents_NonOwner_IgnoresGroupIDParam(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/audit?group_id=42&actor_user_id=777", nil)
	req.Header.Set("X-Caller-User-Id", "123")
	w := httptest.NewRecorder()
	h.listAuditEvents(w, req)
	...
	// actor_user_id param must be ignored; caller's own scope is set via OwnUserID
	if f.ActorUserID != nil {
		t.Errorf("ActorUserID: must be nil in non-owner path, got %v", f.ActorUserID)
	}
	if f.OwnUserID == nil || *f.OwnUserID != 123 {
		t.Errorf("OwnUserID: want 123 (caller), got %v", f.OwnUserID)
	}
}
...
func TestListAuditEvents_ServerOwner_CanFilterByActorUserID(t *testing.T) {
	h, repo := newAuditHandler(123)
	req := httptest.NewRequest(http.MethodGet, "/api/v1/audit?actor_user_id=777", nil)
	req.Header.Set("X-Caller-User-Id", "123")
	w := httptest.NewRecorder()
	h.listAuditEvents(w, req)

	f := repo.captured[0]
	if f.ActorUserID == nil || *f.ActorUserID != 777 {
		t.Errorf("ActorUserID: want 777, got %v", f.ActorUserID)
	}
}
...
func TestListAuditEvents_ServerOwner_NoVisibilityRestriction(t *testing.T) {
	...
	if f.OwnUserID != nil {
		t.Errorf("OwnTgID: server owner should not have scope restriction, got %v", f.OwnUserID)
	}
}
