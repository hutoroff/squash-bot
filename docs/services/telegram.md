...
Nearby regression anchors include `commands_test.go`, `venue_handlers_test.go`, `newgame_handlers_test.go`, `result_handlers_test.go`, `webhook_test.go`, and `bot_coalescer_test.go`. Handler tests are partial coverage; the typed client has no dedicated test file at the assessment baseline. See [invariants](../invariants.md) and [verification](../development.md).
