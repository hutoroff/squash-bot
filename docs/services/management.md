...
Integration fixtures use [SetupTestDB, Truncate, and CreateTestUser](../../internal/testutil/testdb.go). There is no `TruncateTables` helper. Fixtures must create canonical identities and satisfy current foreign keys; do not copy stale `tests/e2e` constructors.

Start with focused service/API tests, then integration tests for persistence/transactions. [Invariants](../invariants.md) identifies nearby tests and gaps. Docker-unavailable success exits are still a known verification limitation; [Development](../development.md) explains how to report them. Configuration defaults live in [config.go](../../internal/config/config.go), operator descriptions in the [README](../../README.md#environment-variables).
