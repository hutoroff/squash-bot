...
- At midnight when booking opens, the bot auto-books courts for each configured preferred time (if `SPORTS_BOOKING_SERVICE_URL` and `preferred_game_times` are set), stores one result per slot, immediately creates an **unpublished** game in the DB (invisible to group members), and silently DMs group admins with the outcome
...
| Halfway court check        | ±2m30s of midpoint     | Fires halfway between when courts were booked and the grace-period deadline. Releases half (rounded down) of the currently-unneeded courts. |
