...
Everything above can also be managed in the web UI (port 8082) — the bot's menus and the web UI edit the same data, so use whichever you prefer. The preventive-cancellation point is Web-only; the bot's venue menus do not expose it and leave the stored value untouched.
