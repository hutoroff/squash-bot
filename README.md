...
| `EVERSPORTS_SPORT_UUID`     | No       | `b388b6e6-…`      | UUID of the sport used when creating bookings (`POST /matches`), resolving courts for `GET /matches`, and listing courts (`GET /courts`). Defaults to the well-known Eversports squash UUID. Only change this for non-squash sports. |
...
| `EVERSPORTS_SPORT_ID`       | No       | _(empty)_         | Numeric sport ID from the booking calendar (e.g. `496` for squash at your facility). Required for `GET /api/v1/eversports/matches` and `GET /api/v1/eversports/courts`. Find it in the `sport[id]` field of the `/api/booking/calendar/update` request body in browser DevTools. |
...
| `POST` | `/api/v1/eversports/matches`          | Yes  | Create a court booking. Body: `{"courtUuid":"…","start":"…","end":"…"}` (RFC 3339). Returns `{"bookingUuid":"…","bookingId":…,"matchId":"…"}` (`matchId` omitted if match creation failed). Requires `EVERSPORTS_FACILITY_UUID` and `EVERSPORTS_SPORT_UUID`.                                                                                                                                                                                                                  |
...
EVERSPORTS_EMAIL=you@example.com \
  ...
  EVERSPORTS_SPORT_ID=496 \
  EVERSPORTS_SPORT_UUID=b388b6e6-69de-11e8-bdc6-02bd505aa7b2 \
