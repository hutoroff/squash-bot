...
| `EVERSPORTS_SPORT_UUID`     | No       | `b388b6e6-…`      | UUID of the sport used when creating bookings and for `GET /api/v1/eversports/courts`. Defaults to the well-known Eversports squash UUID. |
| `EVERSPORTS_FACILITY_SLUG`  | No       | _(empty)_         | Facility slug from the venue URL on eversports.de (e.g. `squash-house-berlin-03`). Required for `GET /api/v1/eversports/courts`. |
| `EVERSPORTS_SPORT_ID`       | No       | _(empty)_         | Numeric sport ID from the booking calendar (e.g. `496` for squash at your facility). Required for `GET /api/v1/eversports/courts`. Find it in the `sport[id]` field of the `/api/booking/calendar/update` request body in browser DevTools. |
